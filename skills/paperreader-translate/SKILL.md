...
编译失败时读取 `compile` 返回的诊断报告与源码快照，按 [编译修复流程](references/latex.md#编译失败后的修复) 在当前会话中迭代处理；日志行号仅用于定位，根因可能在导言区或被引用文件。需要图表导航或核对图表页码时，按 [图表索引](references/figures.md) 从实际 PDF 分别定位原文和译文，不能沿用原文页码。
